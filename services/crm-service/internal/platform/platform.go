// Package platform wraps shared infrastructure concerns for CRM service.
package platform
...
func DBConfigFromEnv(prefix string) DBConfig {
	...
	if dsn == "" {
		dsn = "postgres://rinco:rinco_dev_password@localhost:5432/rinco_crm?sslmode=disable"
	}
	u, err := url.Parse(dsn)
	if err != nil {
		return DBConfig{Host: "localhost", Port: 5432, User: "rinco", Password: "rinco_dev_password", Database: "rinco_crm", SSLMode: "disable"}
	}
