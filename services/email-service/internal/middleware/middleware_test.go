// Tests for email-service middleware.
package middleware
...
func TestExtra_TenantMW_AdminTrue(t *testing.T) {
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
...
func TestExtra_AuthMW_NoTenantNoAuth(t *testing.T) {
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)
...
func TestExtra_AuthMW_WithAuthHeader(t *testing.T) {
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	...
	if !called {
		t.Error("next should be called with auth header")
	}
}
