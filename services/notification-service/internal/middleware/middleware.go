// Package middleware provides Echo middleware for the notification service.
package middleware
...
var (
	httpReqs = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "HTTP requests handled by notification service",
...
// SecurityHeadersMW applies conservative headers.
...
// TenantMW extracts X-Tenant-ID / X-User-ID headers.
func TenantMW() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			c.SetRequest(c.Request().WithContext(platform.TenantToCtx(
				c.Request().Context(),
				tenant,
...
// AuthMW requires a tenant context.
...
// RateLimitMW applies a Redis-backed rate limit. No-op when rdb is nil.
