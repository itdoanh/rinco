// Tests for dynamic-model-service platform.
package platform
...
func TestGetenv_Default(t *testing.T) {
	os.Unsetenv("DYN_TEST_VAR")
	if got := Getenv("DYN_TEST_VAR", "default"); got != "default" {
		t.Errorf("got %s, want default", got)
	}
}

func TestGetenv_Set(t *testing.T) {
	os.Setenv("DYN_TEST_VAR", "value")
	defer os.Unsetenv("DYN_TEST_VAR")
	if got := Getenv("DYN_TEST_VAR", "default"); got != "value" {
		t.Errorf("got %s, want value", got)
	}
}

func TestGetenv_EmptyFallback(t *testing.T) {
	os.Setenv("DYN_TEST_VAR", "")
	defer os.Unsetenv("DYN_TEST_VAR")
	if got := Getenv("DYN_TEST_VAR", "default"); got != "default" {
		t.Errorf("got %s, want default", got)
	}
...
func TestGetenvInt_Default(t *testing.T) {
	os.Unsetenv("DYN_TEST_INT")
	if got := GetenvInt("DYN_TEST_INT", 42); got != 42 {
		t.Errorf("got %d, want 42", got)
	}
}

func TestGetenvInt_Set(t *testing.T) {
	os.Setenv("DYN_TEST_INT", "100")
	defer os.Unsetenv("DYN_TEST_INT")
	if got := GetenvInt("DYN_TEST_INT", 42); got != 100 {
		t.Errorf("got %d, want 100", got)
	}
}

func TestGetenvInt_Invalid(t *testing.T) {
	os.Setenv("DYN_TEST_INT", "not-a-number")
	defer os.Unsetenv("DYN_TEST_INT")
	if got := GetenvInt("DYN_TEST_INT", 42); got != 42 {
...
func TestInitTracer_EmptyEndpoint(t *testing.T) {
	tp, err := InitTracer(context.Background(), "", "production")
	if err != nil {
...
func TestInitTracer_DevEnv(t *testing.T) {
	tp, err := InitTracer(context.Background(), "http://otel-collector:4317", "development")
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	if tp != nil {
		t.Error("expected nil provider for development env")
	}
}
