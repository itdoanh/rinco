// Package platform wraps the shared infra concerns (logging, config, DB pool,
// middleware helpers) used by all RINCO services. It is intentionally minimal:
// each service is free to bypass these wrappers and use the underlying libraries
// directly.
package platform
...
// Env returns the ENV env var (development by default).
func Env() string {
...
// GetenvInt reads int env, falling back to def on parse error.
...
// DBConfig holds the Postgres pool configuration.
...
// DBConfigFromEnv populates from env vars (AUTH_DATABASE_URL or PG_*).
func DBConfigFromEnv(prefix string) DBConfig {
	...
	u, err := url.Parse(dsn)
	if err != nil {
		return DBConfig{Host: "localhost", Port: 5432, User: "rinco", Password: "rinco_dev_password", Database: "rinco", SSLMode: "disable"}
	}
...
// TenantFromContext extracts tenant_id (empty if missing).
...
// IsAdminFromContext returns whether the current context is super-admin.
...
// TraceFromContext returns the current trace id.
