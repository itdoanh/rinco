...
func newEcho(cfg config, srv *handler.Server, pool *pgxpool.Pool, valkey *redis.Client) *echo.Echo {
	...
	v1.GET("/pages", srv.ListPages, middleware.EditorAuth)
	v1.PUT("/pages/:id", srv.UpdatePage, middleware.EditorAuth)
	v1.DELETE("/pages/:id", srv.DeletePage, middleware.EditorAuth)
	v1.GET("/pages/:id", srv.GetPage, middleware.EditorAuth)
	v1.GET("/blocks", srv.ListBlocks)
	v1.POST("/forms/submit", srv.FormsSubmit)
